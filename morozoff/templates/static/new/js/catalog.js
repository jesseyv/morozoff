...
function catalog()
{
	...
	this.makePageNav = function() {
		...
		if(objCatalog.pages.showPrev == true) {
			$('#bigLeftArraw').show();
			content = content + "<div id='leftArrowId' class='arrow leftArrow'><img src='/img/catalog.list/big_left.png' alt='' title='Вперед'/></div>";
		}
		else {
			$('#bigLeftArraw').hide();
		}
		var content = content + "<div class='floatLeft'>\
			" + objCatalog.pages.page + " из " + objCatalog.pages.pagesCount + "<span style='color:#f0f0f0'>&nbsp;&nbsp;|&nbsp;</span>\
			</div>";
		...
		if(objCatalog.pages.showNext == true) {
			$('#bigRightArraw').show();
			content = content + "<div id='rightArrowId' class='arrow rightArrow'><img src='/img/catalog.list/big_right.png' alt='' title='Вперед'/></div>";
		}
		else {
			$('#bigRightArraw').hide();
		}
	...
	this.setJew = function(jew) {
		...
		var price = jew.price + 'р';
		if(jew.price2 > 0 && moscowGeoCity) {
			jew.price = jew.price2;
			price = jew.price2 + 'р <div class="priceValueConnent">Цена действует в фирменных салонах города Москвы</div>';
		}
		// console.log(seoPage);
		...
		if(Math.floor(jew.old_price) > Math.floor(jew.price)) {
			oldPrice = '<div class=\'oldPrice\'>' + jew.old_price + ' р.</div>';
			if(seoTemplate) {
				oldPriceFirst = '<div style="margin-top: 10px;" class=\'oldPriceFirst\'>' + jew.old_price + ' р.</div>';
			}
			else {
				oldPriceFirst = '<div style="margin-top: 10px;" class=\'oldPrice\'>' + jew.old_price + ' р.</div>';
			}
		...
		if(jew.sprice && jew.quantity) {
			basketButton = '<div><a href=\'\' class=\'addInBasketLink\' onclick="addPropductInBasket(this); return false;">В КОРЗИНУ</a></div>';
		...
		<div class='innerContainer' id='pr" + jew.id + "'>\
			<div class='show'>\
				<div class='decription'>\
					<div class='seoDetailText' style='display: block;'> Код изделия: " + jew.code + "<br />" + jew.text + "</div>"
					+ oldPrice +
					"<div class='showPrice '>" + price + "</div>"
					+ basketButton +
					 "<div><a href='/shops/locator.php'>АДРЕСА САЛОНОВ</a></div>\
				</div>\
			</div>\
			<div class='contain'>\
				<div class='photo'>\
				...
				<div class='pricesContainer" + withPriceClass +"'>"
					+ oldPriceFirst +
					"<div class='price'>"+addClassOrd + jew.price + " р.</div>\
				</div>\
		</div>";
	...
	// Выбирается или отменяется фильтр
	// Эта функция из старого скрипта подлежит переписыванию
	this.selectFilter = function(objField, objItem, text) {
		...
		var title = false;
		// Если фильтр уже был выбран - отменяем его
		...
		// Если выбран фильтр - проверяем ширину фильтр и если нужно делаем отступ
		if(objItem) {
		...
		// Если название фильтр в 2 строки, то делаем отступ
		var paddingTop = 5;
		var paddingBottom = 0;
		if(title.height() > 15) {
			paddingTop = 0;
...
// {
	...
	// this.defaultMatrix = '#catalogAjax';
	// this.seoMatrix = '#seoListContainer';
	// this.allMatrix = '#catalogAjaxAllJews';
	...
	// this.setSelectFields = function()
	// {
	
	// }
	
	// this.hideFirstBlocks = function()
	// {
		
	// }
	
	// this.addBlockToAll = function(startId, page)
	// {
		// var html = this.getCatalogBlockHtml(startId, page);
		// this.arrBlocks[this.arrBlocks.length] = $(this.allMatrix).append(html);
	// }
	
	// this.showMatrixAll  = function()
	// {
		// $(this.allMatrix).fadeIn(300);
	// }
	
	// this.clearAllBlock = function()
	// {
		// $(this.allMatrix).empty();
	// }
	
	
	// this.hideDefaultMatrix = function()
	// {
		// $(this.defaultMatrix).hide();
	// }
	
	// this.hideSeoMatrix = function()
	// {
		// $(this.seoMatrix).hide();
	// }
	...
	// this.setJew = function(jew) {
		...
		// <div class='innerContainer' id='pr" + jew.id + "'>\
			// <div class='show'>\
				// <div class='decription'>\
					 // Код изделия: " + jew.code + "<br />" + jew.text + "\
					 // <br />\
					 // <br />\
					 // <br />\
					 // <div class='showPrice'>" + jew.price + " р.</div>\
				// </div>\
			// </div>\
			// <div class='contain'>\
				// <div class='photo'>\
						// <a href='" + jew.url + "'><img src='" + jew.photo + "' alt='" + jew.text + "' title='" + jew.text + "'/></a>\
				// </div>\
				// <div class='price'>" + jew.price + " р.</div>\
			// </div>\
		// </div>";
		
		// var id = jew.number;
		
		// var contaner = $('#jewerly' + id);
	...
	// this.getCatalogBlockHtml = function(startId, page)
	// {
		// var content = '';
		// if(!page) {
			// page = 1;
		...
		// content = '\
			// <table id="pageNum' + page + '">\
				// <tr>\
					// <td id=\'jewerly' + startId + '\' colspan=\'2\' rowspan=\'2\' class="product big popright"></td>\
					// <td id=\'jewerly' + (startId + 1) + '\' class="product popRightSmall"></td>\
					// <td id=\'jewerly' + (startId + 2) + '\' class="product popLeftSmall"></td>\
					// <td id=\'jewerly' + (startId + 3) + '\' class="product popLeftSmall"></td>\
					// <td id=\'jewerly' + (startId + 4) + '\' class="product popLeftSmall"></td>\
				// </tr>\
				// <tr>\
					// <td id=\'jewerly' + (startId + 5) + '\' class="product popRightSmall"></td>\
					// <td id=\'jewerly' + (startId + 6) + '\' class="product popLeftSmall"></td>\
					// <td id=\'jewerly' + (startId + 7) + '\' colspan=\'2\' rowspan=\'2\' class="product big popleft"></td>\
				// </tr>\
				// <tr>\
					// <td id=\'jewerly' + (startId + 8) + '\' class="product popRightSmall"></td>\
					// <td id=\'jewerly' + (startId + 9) + '\' class="product popRightSmall"></td>\
					// <td id=\'jewerly' + (startId + 10) + '\' class="product popRightSmall"></td>\
					// <td id=\'jewerly' + (startId + 11) + '\' class="product popLeftSmall"></td>\
				// </tr>\
			// </table>\
		// ';
	...
	// {
		...
		// var pageTable = $('#pageNum' + page);
		// var pageTableOffset = pageTable.offset();

		// $(window).scrollTop(pageTableOffset.top);
	// }
// }

// function catalogData()
// {
	// this.scriptName = '/catalog/index.php';
	// this.request = false;
	// this.events = {
		// getDataSuccess : {
			// action : false,
	...
	// {
		...
		// if(action && !this.events.getDataSuccess.action) {
			// this.createEvent('getDataSuccess', action);
		// }
		
		// // alert(this.request.page);
		...
		// $.ajax({
			...
			// {
				...
				// catalogManager.catalogData.ajaxData = data;
				// catalogManager.catalogData.events.getDataSuccess.action();
			// },
			// error: function()
			// {
	...
	// this.createEvent = function(event, action)
	// {
		// this.events[event].action = action;
	// };
	...
	// {
		...
		// }
		// this.jsonData = $.parseJSON(this.ajaxData);
		// return this.jsonData;
	// };
...
// function catalogBind()
// {
	// this.bindShowAll = function()
	// {
